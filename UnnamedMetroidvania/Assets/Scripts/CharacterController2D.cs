...
public class CharacterController2D : MonoBehaviour
{
    ...
    [SerializeField] private Transform m_WallCheck;								// en position med en empty gameobject brugt til at tjekke for v�g
	...
	private bool m_Walled;              // om playeren er mod en v�g eller ik
	...
	private void FixedUpdate()
	{
        ...
        Collider2D[] WallColis = Physics2D.OverlapCircleAll(m_WallCheck.position, k_WalledRadius, m_WhatIsGround); //samme check om ground men for v�g
        for (int i = 0; i < WallColis.Length; i++)
        {
            if (WallColis[i].gameObject != gameObject)
            {
                m_Walled = true;
				if (m_Rigidbody2D.velocity.y < 0) //tilf�jet s�tning s� man falder langsommere n�r man st�r op ad en v�g
				{
	...
	public void Move(float move, bool crouch, bool jump, bool Dashing)
	{
        ...
        if (m_Grounded) //reset dash og jump n�r man lander
		{
			DashCount = 1;
			JumpCount = 2;
		}
		...
		if (DashCount >= 1 && Dashing) //starter dash hvis man trykker p� dash knappen og dashcount er lig med eller over 1
		{
            m_Rigidbody2D.AddForce(new Vector2(m_Rigidbody2D.transform.localScale.x * m_dashForce, 0)); // tilf�rer en force til playerens rigidbody i retningen den vender pga. flip()
            DashCount--;
        }
		if (m_Walled && jump && !m_Grounded) //n�r man er mod en v�g og ikke p� jorden laver man et wallhop
		{
			m_Rigidbody2D.AddForce(new Vector2(-m_Rigidbody2D.transform.localScale.x * 300f, m_JumpForce));
		}
		if (m_Walled && m_Grounded && jump) //man st�r i et hj�rne hopper man normalt
		{
            m_Grounded = false;
            m_Rigidbody2D.AddForce(new Vector2(0f, m_JumpForce));
            JumpCount--;
